...
int main(int argc, string argv[]) //user must use ./caesar (key they wish to use to cipher their text)
{

int k = atoi(argv[1]); //convert each array of the argument into interger as they are string

    if (argc > 2 || argc < 2 )// check to see if user only input 1 argument
    {
        printf("usage : ./caesar key\n");
        return 1;
    }

    if (only_digits(argv[1]))// check to see that the argument/key must be only number digits (positive)
        {
        return 1;
        }
